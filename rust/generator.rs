...
pub trait Generator {
    ...
    fn fxchg(&mut self, dst: u8, s1: u8);
...
pub fn powi<T: Generator>(ir: &mut T, dst: u8, s1: u8, power: i32) {
    if power == 0 {
...
pub fn powi_mod<T: Generator>(ir: &mut T, dst: u8, s1: u8, power: i32, modulus: u8) {
    assert!(dst != RET && s1 != RET);
...
pub fn fmod<T: Generator>(ir: &mut T, dst: u8, s1: u8, s2: u8) {
    assert!(dst != RET && s1 != RET && s2 != RET);
    ir.divide(RET, s1, s2);
    ir.floor(RET, RET);
    ir.times(RET, RET, s2);
    ir.minus(dst, s1, RET);
...
pub fn setup_call_unary<T: Generator>(ir: &mut T, s1: u8) {
    if s1 != LEFT {
        ir.fmov(LEFT, s1);
    };
}

pub fn setup_call_binary<T: Generator>(ir: &mut T, s1: u8, s2: u8) {
    if s1 == RIGHT && s2 == LEFT {
        ir.fxchg(RIGHT, LEFT);
    } else if s2 == LEFT {
        ir.fmov(RIGHT, LEFT);
