...
def powi_mod(**args):
    ...
    b1 = binom(x, y, 7, 4)
    b2 = binom(x, y, 8, 5)
    p = E(f"rem(({b1}) ^ 5, 65537) + rem(({b2}) ^ (4^x), 65537)")
...
def fact(**args):
    def factorial(x, n):
        if n == 0:
            ...
            return E(f"if(geq(x, {n}), {n}, 1)") * factorial(x, n - 1)
...
def sumprod(**args):
    fact = compile_func([z], E("Product(y, (y, 1, z))"), **args)
    f = func([x], E("Sum(x^z / F(z), (z, 0, 50))"), **args, defuns={F: fact})
