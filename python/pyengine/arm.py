...
class Arm(assembler.Assembler):
    ...
    def ldp_d(self, rd, rd2, rn, of7):
        ...
        self.append_word(0x6d400000 | self.rd(rd) | self.rd2(rd2) | self.rn(rn) | self.of7(of7))
        return self

    def ldp_x(self, rd, rd2, rn, of7):
        # ldr x(rd), x(rd2), [x(rn), ofs]
        self.append_word(0xa9400000 | self.rd(rd) | self.rd2(rd2) | self.rn(rn) | self.of7(of7))
        return self
    
    def stp_d(self, rd, rd2, rn, of7):
        # stp d(rd), d(rd2), [x(rn), ofs]
        self.append_word(0x6d000000 | self.rd(rd) | self.rd2(rd2) | self.rn(rn) | self.of7(of7))
        return self

    def stp_x(self, rd, rd2, rn, of7):
        # stp x(rd), x(rd2), [x(rn), ofs]
        self.append_word(0xa9000000 | self.rd(rd) | self.rd2(rd2) | self.rn(rn) | self.of7(of7))
        return self
    ...
    def add_lsl(self, rd, rn, rm, shift):
        ...
        self.append_word(0x8b000000 | self.rd(rd) | self.rn(rn) | self.rm(rm) | self.imm(shift & 3))
        return self
    
    def sub_lsl(self, rd, rn, rm, shift):
        # sub x(rd), x(rn), x(rm), LSL #shift
        self.append_word(0xcb000000 | self.rd(rd) | self.rn(rn) | self.rm(rm) | self.imm(shift & 3))
        return self                
