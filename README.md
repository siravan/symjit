...
For the rust engine, the result of different `compile` functions is a python object, say `f`, that encapsulates the underlying compiled code. When we call `f(...)`,  `f.__call__` is called with the arguments. Then,  `__call__` checks the type of arguments (scalar vs vector), package the input, and calls the correct compiled routine via the respective rust routines. All these actions have an overhead. The overhead is acceptable if the compiled function is large and complex, but the overhead becomes relatively too high if the function is simple and light. In this situation, it is faster to directly call the underlying compiled code. If the following conditions hold, it is possible to do so:

1. The output is a single expression.
2. There are zero to eight input arguments.
3. There is no parameter.

We can access the fast function by called `f.fast_func()`. The result is a `ctypes.CFUNCTYPE`-generated foreign function. For example, the integration example above can be written as
