...
The vectorized version generated by the Rust backend uses SIMD instructions if possible (currently supporting AVX instructions in X86-64 processors) and parallel processing when supported (using [Rayon rust crate](https://docs.rs/rayon/latest/rayon/)). SIMD code should improve the performance up to 4x for certain tasks (using 256-bit registers that encode and operate on four doubles simultaneously). It is possible to disable SIMD and parallel processing (multi-threasing) by passing `use_simd=False` and `use_threads=False` to `compile_func`.
...
Note that this is the output from an older version and the more recent versions have more complex prologue and epilogue. 
