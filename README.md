...
Introduced in version 1.5.0, *Symjit* has two different code-generating backends. The default is a Rust library with minimum external dependency. The second backend is in plain Python with dependency  only on the Python standard library and numpy. Both backends generate AMD64 (aka x86-64) and ARM64 (aka aarch64) machine codes on Linux, Windows, and Darwin (MacOS) platforms. Further architectures and operating systems (e.g., RISC V) are planned.

As of version 2.0.0, the Rust backend generates AVX-compatible code by default for x64-86/AMD64 processors but can downgrade to SSE2 instructions if the processor does not support AVX or explicitely asked to do so by passing `ty='amd-sse'` to the compile functions (see below). In version 1, the default was SSE instructions. Note that SSE2 instructions were introduced in 2000, which means that virtually all current 64-bit x86-64 processors support it. AVX instruction set was introduced in 2011; therefore, the majority of processors support it. The Python backend uses only AVX instructions. 

On ARM64 processors, both the Rust and Python backends generate code for aarch64 instrucion set. 

ARM32 and IA32 are not supported. 
...
For the rust engine, the result of different `compile` functions is a python object, say `f`, that encapsulates the underlying compiled code. When we call `f(...)`,  `f.__call__` is called with the arguments. Then,  `__call__` checks the type of arguments (scalar vs vector), package the inputs accordingly, calls the correct compiled routine via the respective rust routines, and finally, formats the return values. All these actions have an overhead. The overhead is acceptable if the compiled function is large and complex, but the overhead becomes relatively too expensive if the function called is simple and light. In this situation, it is faster to directly call the underlying compiled code. If the following conditions hold, it is possible to do so:

1. The output is a single scalar expression.
2. There are zero to eight scalar input arguments.
3. There is no parameter.

In most cases, *Symjit* detects that a function can be switched to a fast function and does it automatically behind the scene. However, there are situations when using the fast function directly improves the performance. For example, passing functions to scipy integration functions (`quad`, `nquad`, `dbpquad`, `tplquad`). To assist this, we can access the fast function by called `f.fast_func()`. The result is a `ctypes.CFUNCTYPE`-generated foreign function. For example, the integration example above can be written as
...
Some points. First, we pass a lambda function to `nquad` because of the pecularities of `nquad` (and other scipy integration routines) and the expected signature of the foreign functions. We plan to generate the correct signature in a future version. Second, the lifetime of the fast function is linked to `f`. If `f` goes out of the scope and is garbage-collected, the fast function becomes invalid. Therefore, never store the resulting fast function separately from the parent `f`. This is why we add `integrate` function to provide a scope for the fast function. 


## Exponentiation to an Integer Power and Modular Exponentiation

Polynomial manipulation over various finite and infinite fields, such as &Zopf;_p and &Zopf;, is the cornerstone of computer algebra systems. *Symjit* is primarily designed as a bridge between sympy and numerical libraries (numpy, scipy, ...) and as such focuses of floating point calculations. However, to assist with sympy integer calculations, version 2 adds the capacility to detect and emit special codes for integer exponentiation and modular exponentiation. It should be noted that doubles can represent integers accurately up to 2**53 = 9007199254740992. 
...
* `amd-sse`: generates 64-bit AMD64/x86-64 SSE code. It expects a minimum SSE2.1 spec, which should be easily fulfilled by all except the most ancient processors!
...
* `bytecode`: this is a generic bytecode evaluator as a fallback option in case the instruction set is not supported.
